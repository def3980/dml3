 ...
 */
class sfGenerateModuleFullBootstrap3Task extends sfGeneratorBaseTask {
    ...
    protected function configure() {
        $this->addArguments(array( // 1 == sfCommandArgument::REQUIRED, 2 == sfCommandArgument::OPTIONAL
            ...
            new sfCommandArgument('module', 2, 'El nombre del modulo', 'bootstrap3'),
        ));
        $this->namespace           = 'generate';
        $this->name                = 'module-full-bootstrap3';
        $this->briefDescription    = '>> Genera un modulo completo sobre Bootstrap3';
        $this->detailedDescription = <<<EOF
La tarea [generate:module-full-bootstrap3|INFO] crea una estructura de 
directorio completa sobre Bootstrap3 en una aplicacion existente:

  [./symfony generate:module-full-bootstrap3 frontend nombre_modulo|INFO]

El nombre del modulo se puede omitir ya que "bootstrap3" esta por defecto
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array()) {
        ...
        if (is_readable(sfConfig::get('sf_data_dir').'/skeleton/module-bootstrap3')) {
            $skeletonDir = sfConfig::get('sf_data_dir').'/skeleton/module-bootstrap3';
        } else {
            $skeletonDir = dirname(__FILE__).'/skeleton/module-bootstrap3';
        }
